...
getSubstratesPerToolForGivenPalletIDs <- function(pirData,palletIDs,toolColumn ,toolName){
        ...
        toolCriteria <- interp(~column==tool, tool=toolName, column=as.name(toolColumn))
...
getSubstratesBalanceForGivenPalletIDS <- function(pirData, palletIDs){
        balance <- pirData %>%
                filter(GLS_BATCH_NR %in% palletIDs) %>%
                select(SUBSTRATE_ID, GLS_BATCH_NR, GCI_TIME_END, GCI_TOOL, GCI_RESULT, GMS_TIME_END, GMS_TOOL, GMS_RESULT, AOI_TIME_END, AOI_TOOL, AOI_RESULT)
